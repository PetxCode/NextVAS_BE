...
export const updateProfile = async (req: any, res: Response) => {
  try {
    ...
    if (term) user.term = term;
    if (subject) user.subject = subject;
    if (assimilationLevel) user.assimilationLevel = assimilationLevel;
    if (voiceName) user.voiceName = voiceName;
...
export const recordSession = async (req: any, res: Response) => {
  try {
    ...
    const logIndex = user.activityLog.findIndex((log: any) => log.date === today);
    
    if (logIndex > -1) {
      if (minutes) user.activityLog[logIndex].minutes += minutes;
      if (isNewSession) user.activityLog[logIndex].sessions += 1;
    } else {
